...
resource "azurerm_lb" "lb" {
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      subnet_id                                          = can(frontend_ip_configuration.value.subnet.key) ? var.remote_objects.virtual_network[frontend_ip_configuration.value.subnet.vnet_key].subnets[frontend_ip_configuration.value.subnet.key].id : try(frontend_ip_configuration.value.subnet.id, null)
      ...
      public_ip_address_id                               = can(frontend_ip_configuration.value.public_ip_address.key) ? var.remote_objects.public_ip_addresses[frontend_ip_configuration.value.public_ip_address.key].id : try(frontend_ip_configuration.value.public_ip_address.id, null)
